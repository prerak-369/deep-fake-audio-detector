...
class EpisodicMemory:
    ...
    def store_detection(self, audio_path: str, filename: str, detection: dict, context: dict) -> str:
        ...
        try:
            crud.create_case(db, {
                ...
                "input_type":    context.get("input_type", "voice"),
            })
        finally:
            db.close()
        return case_id

    def update_case(self, case_id: str, analysis: str, report: dict, risk: dict, sentiment: str | None = None) -> None:
        """Append agent analysis, report path, risk metadata, and sentiment to an existing case."""
        db = SessionLocal()
        try:
            updates = {
            ...
            crud.update_case_analysis(db, case_id, updates)
        finally:
            db.close()
    ...
    def get_recent_by_submitter(self, submitter: str, days: int = 30) -> list:
        """Return recent cases for a given submitter (customer history)."""
        db = SessionLocal()
        try:
            return crud.get_cases_by_submitter(db, submitter, days)
        finally:
            db.close()
