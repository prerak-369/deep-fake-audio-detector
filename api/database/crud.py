...
def get_cases_by_submitter(db: Session, submitter: str, days: int = 30) -> list[dict]:
    """Return cases for a submitter (customer) within the last N days."""
    cutoff = (datetime.now(timezone.utc) - timedelta(days=days)).isoformat()
    rows = (
        db.query(AudioCase)
        .filter(AudioCase.submitter == submitter, AudioCase.timestamp >= cutoff)
        .order_by(AudioCase.timestamp.desc())
        .all()
    )
    return [_row_to_dict(r) for r in rows]
